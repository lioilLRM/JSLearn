...
$('#config_search').click(function() {
  const artname = $('#activity-name').text()
  const baseurl = 'https://weixin.sogou.com/weixin?type=2&query='
  const trueurl = baseurl + artname
...
if (state.url_state === true) {
  const linkReg = /msg_cdn_url = "(.*)"/gi
  const data = $('*').html()
  const url = linkReg.exec(data)
  const trueurl = url[1]
  // 跳转
  ...
  // 点击按钮复制封面链接到剪切板
  const clipboard = new ClipboardJS('.myclipboard') // 要使用 clipboard.js 需要声明一个clipboard实例
  $('#config_preview').attr('data-clipboard-text', trueurl)
  $('#config_preview').click(function() {
    const clipboard = new ClipboardJS('#config_preview') // 要使用 clipboard.js 需要声明一个clipboard实例
    alert('封面链接已复制到剪切板！')
  })
}
// 摘要
