...
;(function () {
  ...
  var menuAll = [
    ...
    urlArr = [
      // 外置翻页规则更新地址分流，以确保更新成功率（记得 connect）
  ...
  window.autoPage = {
    ...
    getNextEP: getNextEP,
    getNextSP: getNextSP,
    getNextEPN: getNextEPN,
    getNextUPN: getNextUPN,
    getNextUP: getNextUP,
    getNextF: getNextF,
    getSearch: getSearch,
    getCookie: getCookie,
  ...
  function registerMenuCommand() {
    ...
    for (let i = 0; i < menuAll.length; i++) {
      ...
      if (menuAll[i][0] === 'menu_disable') {
        // 启用/禁用

        if (menu_disable('check')) {
          // 当前网站在禁用列表中
          menuId[i] = GM_registerMenuCommand(`${menuAll[i][2]}`, function () {
            menu_disable('del')
          })
          return
        } else {
          ...
          if (webType === 0) {
            ...
            console.info(
              '[自动无缝翻页] - 暂不支持当前网页 [ ' +
                location.href +
                ' ]，申请支持: https://github.com/XIU2/UserScript / https://greasyfork.org/zh-CN/scripts/419215/feedback',
            )
            return
          } else if (webType === -1) {
            return
          }
          menuId[i] = GM_registerMenuCommand(`${menuAll[i][1]}`, function () {
            menu_disable('add')
          })
        }
        ...
        menuId[i] = GM_registerMenuCommand(`#️⃣ ${menuAll[i][1]}`, function () {
          customRules()
        })
      } else {
        menuId[i] = GM_registerMenuCommand(
          `${menuAll[i][3] ? '✅' : '❌'} ${menuAll[i][1]}`,
          function () {
            menu_switch(menuAll[i][3], menuAll[i][0], menuAll[i][2])
          },
        )
      }
    }
    menuId[menuId.length] = GM_registerMenuCommand(
      '💬 反馈失效 / 申请支持',
      function () {
        window.GM_openInTab(
          'https://github.com/XIU2/UserScript#xiu2userscript',
  ...
  function doesItSupport() {
    ...
    let support = false
    end: for (let now in DBSite) {
      // 遍历 对象
      if (!DBSite[now].host) continue // 如果不存在则继续下一个循环
      // 如果是 数组

      if (Array.isArray(DBSite[now].host)) {
        for (let i of DBSite[now].host) {
          // 遍历 数组

          // 针对自定义翻页规则中的正则
          if (typeof i === 'string' && i.slice(0, 1) === '/')
            i = new RegExp(i.slice(1, i.length - 1))
          if (
            (i instanceof RegExp && i.test(location.hostname)) ||
            (typeof i === 'string' && i === location.hostname)
          ) {
            if (self != top) {
              if (!DBSite[now].iframe) continue end
            } // 如果当前位于 iframe 框架下，就需要判断是否需要继续执行
            if (DBSite[now].url) {
              if (typeof DBSite[now].url == 'function') {
                DBSite[now].url()
              } else {
                // 自定义翻页规则时，因为同域名不同页面 url 分开写，所以如果没找到就需要跳出当前数组循环，继续规则循环
                try {
                  if (DBSite[now].url.slice(0, 1) === '/') {
                    // 如果是正则，则对 URL 路径进行匹配
                    if (
                      new RegExp(
                        DBSite[now].url.slice(1, DBSite[now].url.length - 1),
                        'i',
        ...
        // 针对自定义翻页规则中的正则
        if (
          typeof DBSite[now].host === 'string' &&
          DBSite[now].host.slice(0, 1) === '/'
        )
          DBSite[now].host = new RegExp(
            DBSite[now].host.slice(1, DBSite[now].host.length - 1),
          )
        if (
          (DBSite[now].host instanceof RegExp &&
            DBSite[now].host.test(location.hostname)) ||
          (typeof DBSite[now].host === 'string' &&
            DBSite[now].host === location.hostname)
        ) {
          if (self != top) {
            if (!DBSite[now].iframe) continue
          } // 如果当前位于 iframe 框架下，就需要判断是否需要继续执行
          if (DBSite[now].url) {
            if (typeof DBSite[now].url == 'function') {
              DBSite[now].url()
            } else {
              // 自定义翻页规则时，因为同域名不同页面 url 分开写，所以如果没找到就需要继续规则循环
              try {
                if (DBSite[now].url.slice(0, 1) === '/') {
                  // 如果是正则，则对 URL 路径进行匹配
                  if (
    ...
    if (support) {
      console.info('[自动无缝翻页] - 独立规则 网站')
      return 1
    } else if (self != top) {
      return -1
    } else if (
      typeof discuz_uid != 'undefined' ||
      getCSS(
        'meta[name="author" i][content*="Discuz!" i], meta[name="generator" i][content*="Discuz!" i], body[id="nv_forum" i][class^="pg_" i][onkeydown*="27"], body[id="nv_search" i][onkeydown*="27"]',
      ) ||
      getXpath('id("ft")[contains(string(),"Discuz!")]')
    ) {
      console.info(`[自动无缝翻页] - <Discuz!> 论坛`)
      return 2
    } else if (typeof flarum != 'undefined' || getCSS('#flarum-loading')) {
      console.info(`[自动无缝翻页] - <Flarum> 论坛`)
      return 3
    } else if (typeof phpbb != 'undefined' || getCSS('body#phpbb')) {
      console.info(`[自动无缝翻页] - <phpBB> 论坛`)
      return 4
    } else if (
      typeof xn != 'undefined' &&
      getXpath(
        '//footer//a[contains(string(), "Xiuno")] | //link[contains(@href, "xiuno")] | //script[contains(@src, "xiuno")]',
      )
    ) {
      console.info(`[自动无缝翻页] - <Xiuno> 论坛`)
      return 5
    } else if (typeof XF != 'undefined') {
      console.info(`[自动无缝翻页] - <XenForo> 论坛`)
      return 6
    } else if (
      getCSS('head meta[name="generator" i][content="nexusphp" i]') ||
      getXpath('id("footer")[contains(string(), "NexusPHP")]')
    ) {
      console.info(`[自动无缝翻页] - <NexusPHP> 论坛`)
      return 7
    } else if (
      loadMoreExclude(loadMoreExclude1) &&
      getAllCSS(
        '.load-more, .load_more, .loadmore, #load-more, #load_more, #loadmore, .show-more, .show_more',
      ).length === 1
    ) {
      console.info(`[自动无缝翻页] - 部分自带 自动无缝翻页 的网站 1`)
      return 8
    } else if (
      loadMoreExclude(loadMoreExclude2) &&
      getAllXpath(
        '//*[self::a or self::span or self::button or self::div][text()="加载更多" or text()="查看更多"][not(@href) or @href="#" or starts-with(@href, "javascript")]',
      ).length === 1
    ) {
      console.info(`[自动无缝翻页] - 部分自带 自动无缝翻页 的网站 2`)
      return 9
    } else if (
      getCSS('link[href*="/wp-content/" i], script[src*="/wp-content/" i]')
    ) {
      ...
      if (getCSS('.post-page-numbers.current+a')) {
        if (getAllCSS('.entry-content').length == 1) {
          DBSite.wp_article_post.pager.pageE =
            '.entry-content>*:not(.wbp-cbm):not(.page-links):not(.article-paging):not(.entry-pagination):not(.pagination)'
        } else if (getAllCSS('.article-content').length == 1) {
          DBSite.wp_article_post.pager.pageE =
            '.article-content>*:not(.page-links):not(.article-paging):not(.entry-pagination):not(.pagination)'
        } else if (getAllCSS('article').length == 1) {
          DBSite.wp_article_post.pager.pageE =
            'article>*:not(.page-links):not(.article-paging):not(.entry-pagination):not(.pagination)'
        }
        if (DBSite.wp_article_post.pager.pageE != undefined)
          console.info(`[自动无缝翻页] - 部分使用 WordPress 的网站 - 文章内`)
        return 11
      }
      ...
      if (getCSS('a.next, a.next-page')) {
        ...
        getCSS(
          'a[rel="next" i], a[aria-label="Next Page" i], a[aria-label="下一页"], a[rel="下一页"], a[title="下一页"], a[aria-label="下一頁"], a[rel="下一頁"], a[title="下一頁"]',
        )
      ) {
        DBSite.wp_article.pager.nextL =
          'a[rel="next" i], a[aria-label="Next Page" i], a[aria-label="下一页"], a[rel="下一页"], a[title="下一页"], a[aria-label="下一頁"], a[rel="下一頁"], a[title="下一頁"]'
      } else if (
        getCSS('li.next-page > a, li.next > a, li.pagination-next>a')
      ) {
        DBSite.wp_article.pager.nextL =
          'li.next-page > a, li.next > a, li.pagination-next>a'
        ...
        getXpath(
          '//a[contains(text(), "下一页") or contains(text(), "下一頁") or contains(text(), ">") or contains(text(), "next") or contains(text(), "Next") or contains(text(), "NEXT")]',
          getCSS(
            '#nav-below, nav.navigation, nav.paging-navigation, .pagination:not([class*="entry"]), .wp-pagenavi, .pagenavi, nav[role="navigation"]',
          ),
        )
      ) {
        DBSite.wp_article.pager.nextL =
          '//*[self::ul or self::nav or self::div][@id="nav-below" or contains(@class, "navigation") or contains(@class, "pagination") or contains(@class, "pagenavi") or @role="navigation"]//a[contains(text(), "下一页") or contains(text(), "下一頁") or contains(text(), ">") or contains(text(), "next") or contains(text(), "Next") or contains(text(), "NEXT")]'
      }
      ...
      if (DBSite.wp_article.pager.nextL != undefined) {
        if (getAllCSS('main').length == 1) {
          ...
          if (DBSite.wp_article.pager.pageE != undefined) {
            console.info(`[自动无缝翻页] - 部分使用 WordPress 的网站`)
            return 10
          }
        }
      ...
      getCSS('meta[name="template" i][content="handsome" i]') &&
      getCSS('.page-navigator')
    ) {
      console.info(`[自动无缝翻页] - 部分使用 Typecho 的网站 (handsome)`)
      return 12
    } else if (
      getCSS('meta[name="template" i][content="Mirages" i]') &&
      getCSS('.page-navigator')
    ) {
      console.info(`[自动无缝翻页] - 部分使用 Typecho 的网站 (Mirages)`)
      return 13
    } else if (
      getCSS(
        '.stui-page, .stui-page__item, #long-page, .myui-page, .myui-page__item',
      )
    ) {
      console.info(`[自动无缝翻页] - 部分影视网站`)
      return 300
    } else if (getCSS('#page') && getCSS('.module-items')) {
      console.info(`[自动无缝翻页] - 部分影视网站 2`)
      return 301
    } else if (
      getCSS('.ArticleImageBox, .PictureList') &&
      getCSS('.article_page') &&
      getXpath('//div[contains(@class,"article_page")]//a[text()="下一页"]')
    ) {
      console.info(`[自动无缝翻页] - 部分美女图站 - 手机版`)
      return 302
    } else if (
      getCSS(
        'meta[content^=SearXNG i], link[href*=SearXNG i], script[src*=SearXNG i]',
      )
    ) {
      console.info(`[自动无缝翻页] - <SearXNG> 元搜索引擎`)
      return 303
    } else if (
      getCSS('.content > #content') &&
      getXpath('//div[contains(@class,"page_chapter")]//a[text()="下一章"]')
    ) {
      console.info(`[自动无缝翻页] - <笔趣阁 1> 模板的小说网站`)
      return 200
    } else if (
      getCSS('#nr1, #chaptercontent, .Readarea, .ReadAjax_content') &&
      getCSS('#pb_next, #linkNext')
    ) {
      console.info(`[自动无缝翻页] - <笔趣阁 1 - 手机版> 模板的小说网站`)
      return 201
    } else if (
      getCSS('#txt, .txt') &&
      getCSS('#pb_next, .url_next') &&
      getCSS('.chapter-control, .chapter-page-btn')
    ) {
      console.info(`[自动无缝翻页] - <笔趣阁 2> 模板的小说网站`)
      return 202
    } else if (
      (getCSS(
        'meta[name="description" i][content*="小说"], meta[name="description" i][content*="章节"], meta[name="description" i][content*="阅读"]',
      ) ||
        location.hostname.indexOf('biqu') != -1 ||
        document.title.match(/笔趣阁|小说|章/) != null) &&
      getAllCSS(
        '[id="content" i], [class~="content" i], [id="chaptercontent" i], [class~="chaptercontent" i], [id="booktext" i], [id="txtcontent" i], [class~="txtnav" i], [id="txt" i][class~="txt" i]',
      ).length === 1 &&
      getXpath(
        '//a[contains(text(), "下一章") or contains(text(), "下一页") or contains(text(), "下一节")]',
      )
    ) {
      console.info(`[自动无缝翻页] - <笔趣阁 3> 模板的小说网站`)
      return 203
    }
    return 0
  }
    ...
    if (webType != 1) {
      switch (webType) {
        ...
        case 10: // < 部分使用 WordPress 的网站 >
          DBSite.wp_article.url()
          break
        case 11: // < 部分使用 WordPress 的网站 - 文章内 >
          curSite = DBSite.wp_article_post
          break
        case 12: // < 部分使用 Typecho 的网站 (handsome) >
          curSite = DBSite.typecho_handsome
          break
        case 13: // < 部分使用 Typecho 的网站 (Mirages) >
          curSite = DBSite.typecho_mirages
          break
        case 200: // < 所有使用 笔趣阁 1 模板的小说网站 >
          DBSite.biquge1.url()
          break
        case 201: // < 所有使用 笔趣阁 1 - 手机版 模板的小说网站 >
          curSite = DBSite.biquge1_m
          break
        case 202: // < 所有使用 笔趣阁 2 模板的小说网站 >
          DBSite.biquge2.url()
          break
        case 203: // < 所有使用 笔趣阁 3 模板的小说网站 >
          curSite = DBSite.biquge3
          break
        case 300: // < 部分影视网站 >
          curSite = DBSite.yingshi
          break
        case 301: // < 部分影视网站 2 >
          curSite = DBSite.yingshi2
          break
        case 302: // < 部分美女图站 - 手机版 >
          curSite = DBSite.meinvtu_m
          break
        case 303: // < SearXNG 元搜索引擎 >
          document.cookie =
            'infinite_scroll=1; expires=Thu, 18 Dec 2031 12:00:00 GMT; path=/'
          document.cookie =
            'results_on_new_tab=1; expires=Thu, 18 Dec 2031 12:00:00 GMT; path=/'
          break
      }
    }
  }
...
function: {
    ...
    */ //<<< 规则简单说明 >>>
    DBSite = {
      loadmore: {
        url: function (nextL) {
          curSite = DBSite.loadmore
          curSite.pager.nextL = nextL
        },
        pager: {
          type: 2,
          isHidden: true,
      ...
      wp_article: {
        url: () => {
          if (
            !indexOF('/post/') &&
            !getCSS('#comments, .comments-area, #disqus_thread')
          ) {
            curSite = DBSite.wp_article
            // 自适应瀑布流样式
            setTimeout(() => {
              if (
                getCSS(curSite.pager.pageE).style.cssText.indexOf(
                  'position: absolute',
                ) != -1
              ) {
                insStyle(
                  curSite.pager.pageE +
                    '{position: static !important; float: left !important; height: ' +
                    parseInt(getCSS(curSite.pager.pageE).offsetHeight * 1.1) +
                    'px !important;}',
                )
              }
            }, 1500)
      ...
      biquge1: {
        url: () => {
          curSite = DBSite.biquge1
          xs_bF(getAllCSS('.content > #content'), [/<br>.{0,10}秒记住.+$/, ''])
        },
        style:
          'img, .posterror, a[href*="posterror()"], [style*="background"][style*="url("]:not(html):not(body), #content > *:not(br):not(p) {display: none !important;}',
        history: true,
        pager: {
      ...
      biquge1_m: {
        style:
          'img, .posterror, .show-app2, a[href*="posterror()"], [onclick*="location.href"], [style*="background"][style*="url("]:not(html):not(body), #nr1>*:not(br):not(p), #chaptercontent>*:not(br):not(p), .Readarea>*:not(br):not(p), .ReadAjax_content>*:not(br):not(p) {display: none !important;}',
        history: true,
        pager: {
      ...
      biquge2: {
        url: () => {
          if (isMobile() || getCSS('.chapter-page-btn') != null) {
            curSite = DBSite.biquge2_m
          } else {
            curSite = DBSite.biquge2
          }
        },
        style:
          'img, .posterror, a[href*="posterror()"], [style*="background"][style*="url("]:not(html):not(body), #txt > *:not(br):not(p) {display: none !important;}',
        history: true,
        pager: {
          type: 6,
      ...
      biquge3: {
        style:
          'img, .posterror, a[href*="posterror()"], [style*="background"][style*="url("]:not(html):not(body), script+div[style="padding:15px;"], p[style*="font-weight:"] {display: none !important;}',
        history: true,
        pager: {
          nextL:
            '//a[contains(text(), "下一章") or contains(text(), "下一页") or contains(text(), "下一节")]',
          ...
          insertP6Br: true,
          replaceE:
            '//a[contains(text(), "下一章") or contains(text(), "下一页") or contains(text(), "下一节")]/parent::*',
        },
        function: {
          bF: xs_bF,
          bFp: [/<br>.{0,10}秒记住.+$/, ''],
      ...
      yingshi: {
        style:
          'div.stui-page__all, div.myui-page__all {display: none !important;}',
        blank: 3,
        pager: {
          nextL:
            '.stui-page li.active+li>a, .stui-page__item li.active+li>a, #long-page .active+li>a, .myui-page .visible-xs+li>a',
      ...
      discuz_guide: {
        pager: {
          nextL:
            'a.nxt:not([href^="javascript"]) ,a.next:not([href^="javascript"])',
      ...
      discuz_thread: {
        thread: true,
        style:
          '.pgbtn, .viewthread:not(:first-of-type)>h1, .viewthread:not(:first-of-type)>ins, .viewthread:not(:first-of-type)>.headactions {display: none;}',
        pager: {
          nextL:
            'a.nxt:not([href^="javascript"]) ,a.next:not([href^="javascript"])',
          pageE: '#postlist > div[id^="post_"], form>.viewthread',
          replaceE:
            '//div[contains(@class,"pg") or contains(@class,"pages")][./a[contains(@class,"nxt") or contains(@class,"next") or contains(@class,"prev")][not(contains(@href,"javascript") or contains(@href,"commentmore"))]]',
          forceHTTPS: true,
        },
        function: {
          bF: src_bF,
      ...
      discuz_m: {
        ...
        pager: {
          nextL:
            '//a[@class="nxt" or @class="next"] | //div[@class="page"]/a[text()="下一页" or contains(text(), ">")]',
          replaceE: '.pg, .page',
          forceHTTPS: true,
          scrollD: 1000,
        },
      ...
      phpbb: {
        ...
        pager: {
          nextL:
            '.pagination li.next a[rel="next"], .topic-actions .pagination strong~a',
      ...
      xiuno: {
        ...
        pager: {
          nextL:
            '//li[@class="page-item"]/a[text()="▶" or text()="»" or contains(text(),">") or contains(text(),"下一页")]',
      ...
      nexusphp: {
        url: () => {
          if (lp == '/torrents.php' || getCSS('table.torrents')) {
            ...
            curSite = DBSite.nexusphp
            curSite.pager.pageE =
              '#outer > table.main~table > tbody > tr:not(:first-of-type)'
          } else if (lp == '/forums.php' && indexOF('action=viewforum', 's')) {
            curSite = DBSite.nexusphp
            curSite.pager.pageE =
              '#outer > table.main+table > tbody > tr:not(:first-of-type):not(:last-of-type)'
        ...
        pager: {
          nextL:
            '//a[./b[contains(text(), "下一页") or contains(text(), ">>")]]',
        ...
        url: () => {
          ...
          if (indexOF(/\/(mods|users)\/\d+/)) {
            if (indexOF('tab=posts', 's')) {
              curSite = DBSite.nexusmods_posts
            } else if (indexOF('tab=user+files', 's')) {
              curSite = DBSite.nexusmods
            }
          } else if (lp !== '/' && getCSS('.pagination a.page-selected')) {
            curSite = DBSite.nexusmods
          }
        },
        blank: 1,
      ...
      bilibili_search: {
        ...
        pager: {
          ...
          pageE:
            '//ul[contains(@class, "video-list")]/li | //script[contains(text(), "window.__INITIAL_STATE__")]',
      ...
      manhuacat: {
        ...
        url: () => {
          if (indexOF(/\/(manga|manhua)\/\d+\/.+\.html/)) {
            if (getCookie('is_pull') == 'true') {
              // 强制关闭 [下拉] 模式
              document.cookie =
                'is_pull=false; expires=Thu, 18 Dec 2031 12:00:00 GMT; path=/' // 写入 Cookie 关闭 [下拉] 模式
        ...
        style:
          '#left, #right, #pull-load, .loading, .pagination, footer {display: none !important;} .img-content > img {display: block !important;margin: 0 auto !important; border: none !important; padding: 0 !important; max-width: 99% !important; height: auto !important;}', // 隐藏不需要的元素，调整图片
        pager: {
          type: 4,
          ...
          replaceE:
            '.comic-detail > .breadcrumb-bar, .comic-detail >h2.h4, .vg-r-data, body > script:not([src])',
          interval: 3000,
          scrollD: 4000,
        },
      }, //         漫画猫 + 漫画飞
      imanhuaw: {
        ...
        url: () => {
          if (getCSS('.mh-search-result')) {
            ...
            curSite = DBSite.imanhuaw
            imanhuaw_init()
          }
        },
        style:
          '#sider-left, #sider-right, .main-left, .main-right, .w996.tc, .title > span {display: none !important;} #qTcms_Pic_middle img {max-width: 110%;height: auto;}',
        pager: {
          type: 4,
      ...
      hicomic: {
        ...
        style:
          '.content {height: auto !important;} .footer, .left_cursor, .right_cursor, .finish {display: none !important;} .content > img {display: block !important;margin: 0 auto !important;}',
        pager: {
          type: 4,
          nextL: hicomic_nextL,
          insertP: ['.content', 3],
      ...
      mangabz: {
        ...
        url: () => {
          if (indexOF(/\/m\d+/)) {
            ...
            if (getCSS('.detail-list-form-more'))
              getCSS('.detail-list-form-more').click()
          } else if (indexOF('/manga-list') || lp == '/search') {
            curSite = DBSite.mangabz_list
          }
        },
        style:
          'body > .container > div:not([id]) {display: none !important;} .top-bar {opacity: 0.3 !important;} #cp_img > img{display: block !important;margin: 0 auto !important; max-width: 99% !important; width: auto !important; height: auto !important;}',
        pager: {
          type: 4,
          nextL: mangabz_nextL,
          insertP: ['#cp_img', 3],
      ...
      mangabz_list: {
        ...
        pager: {
          nextL:
            '//div[contains(@class,"page-pagination")]//a[contains(text(), ">")]',
          pageE: 'ul.mh-list > li',
      ...
      dm5: {
        ...
        style:
          '.view-paging > .container, .view-comment {display: none !important;} .rightToolBar {opacity: 0.3 !important;} #cp_img > img, #barChapter > img{display: block !important;margin: 0 auto !important; max-width: 99% !important; width: auto !important; height: auto !important;} body {overflow: auto !important;}',
        pager: {
          type: 4,
          nextL: dm5_nextL,
      ...
      xmanhua: {
        ...
        style:
          'a.reader-bottom-page {display: none !important;} .header, .reader-bottom {opacity: 0.3 !important;} #cp_img > img{display: block !important;margin: 0 auto !important; max-width: 99% !important; width: auto !important; height: auto !important;}',
        hiddenPN: true,
        pager: {
          type: 4,
          nextL: xmanhua_nextL,
      ...
      coolkeyan: {
        ...
        url: () => {
          if (location.hash.indexOf('/project/') > -1)
            curSite = DBSite.coolkeyan
        },
        style:
          '.q-img {height: auto !important;} .q-img__image {max-height: 1000px !important;} .row.q-my-sm.q-gutter-sm {display: none !important;}',
        pager: {
          type: 4,
      ...
      nsfc: {
        ...
        url: () => {
          if (indexOF('/conclusionProject/')) curSite = DBSite.nsfc
        },
    ...
    if (Object.keys(GM_getValue('menu_customRules', {})).length === 0) {
      DBSite = Object.assign(
        GM_getValue('menu_customRules', {}),
        GM_getValue('menu_rules', {}),
        DBSite,
      )
      DBSite2 = GM_getValue('menu_rules', {})
    } else {
      // 自定义规则 覆盖 同名的外置规则
      let a = GM_getValue('menu_customRules', {}),
        a1 = Object.keys(a),
        b = GM_getValue('menu_rules', {}),
        b1 = Object.keys(b)
  ...
  function getRulesUrl(update = false) {
    ...
    if (typeof GM_getValue('menu_ruleUpdateTime', '') == 'string') {
      update = true
      if (scriptHandler != 'AdGuard') {
        alert(
          '请点击【确定】开始首次获取【外置翻页规则】（大概几秒\n\n在此期间请不要 操作/跳转/关闭 当前网页~\n\n如果不小心没获取成功也没事，可以去脚本菜单中手动【更新外置翻页规则】即可（浏览器右上角 Tampermonkey 扩展图标内的脚本菜单\n\n\n另外，想要【临时暂停翻页】请点击左下角悬浮的【页码】按钮\n\n如果每次打开网页都会看到该提示，说明你的 油猴脚本管理器 存在兼容性问题，请更换其他试试！',
        )
      } else {
        urlArr2 = urlArr
      }
    ...
    if (update) {
      // 手动更新（或安装后首次更新）
      GM_notification({
        text: '🔄 更新外置翻页规则中，请勿操作网页...',
        timeout: 5000,
      })
      getRulesUrl_(true)
    } else if (
      parseInt(+new Date() / 1000) - GM_getValue('menu_ruleUpdateTime', 0) >
      864000
    ) {
      getRulesUrl_()
    }
    ...
    function getRulesUrl_(n = false, url) {
      if (n) {
        url = urlArr2[Math.floor(Math.random() * urlArr2.length)]
      } else {
        url = urlArr[Math.floor(Math.random() * urlArr.length)]
      }
      if (url === undefined) {
        GM_notification({
          text: '❌ 所有更新地址均以试过，已无可用地址，请联系作者解决...',
          timeout: 5000,
        })
        return
      }
        ...
        onload: function (response) {
          try {
            ...
            if (response.response) {
              ...
              if (GM_getValue('menu_page_number')) {
                pageNumber('add')
              } else {
                pageNumber('set')
              } // 显示页码
              ...
              if (n) {
                urlArr2.splice(urlArr2.indexOf(url), 1)
              } else {
                urlArr.splice(urlArr.indexOf(url), 1)
              }
            }
          } catch (e) {
  ...
  if (urlC) {
    ...
    if (webType === 1) {
      window.addEventListener('urlchange', function () {
        ...
        if (
          curSite.history !== false &&
          window.top.document.Autopage_nowUrl === location.href
        ) {
          nowLocation = location.href
          return
        }
        ...
        if (curSite.pager && curSite.pager.type == 5) {
          if (self != top) {
            ...
            if (getCSS('iframe#Autopage_iframe')) {
              getCSS('iframe#Autopage_iframe').remove()
            }
          }
          pausePage = true
  ...
  function discuzForum(m) {
    if (m == 'm') {
      ...
      if (getCSS('a.loadmore')) {
        ...
        curSite = DBSite.discuz_m
        curSite.pager.pageE = '.threadlist > ul > li'
      } else if (getCSS('[id^="normalthread_"]')) {
        curSite = DBSite.discuz_m
        curSite.pager.pageE = '[id^="normalthread_"]:not(.ZDlist)'
      }
      if (curSite.SiteTypeID !== 0 && location.hostname === 'keylol.com') {
        curSite.history = false
        urlC = true
      }
    } else {
      ...
      } else if (getCSS('#waterfall')) {
        //           判断是否为图片模式
        if (!getCSS('#pgbtn, .pgbtn')) {
          //         如果各版块帖子列表已存在这个元素，说明自带了无缝翻页
          curSite = DBSite.discuz_waterfall
          waterfallStyle() // 图片模式列表样式预处理
        }
      } else {
  ...
  function discuzThreadM() {
    if (getCSS('[id^="pid"]')) {
      curSite = DBSite.discuz_m
      curSite.pager.pageE = '[id^="pid"], [id^="pid"]+div:not([id="post_new"])'
    } else if (getCSS('[id^="post_"]')) {
      curSite = DBSite.discuz_m
      curSite.pager.pageE = '[id^="post_"]'
    }
    if (curSite.SiteTypeID !== 0 && location.hostname === 'keylol.com') {
      curSite.history = false
      urlC = true
  ...
  function discuz_() {
    if (getCSS('body[id="nv_forum"][class^="pg_"][onkeydown*="27"]')) {
      switch (
        getCSS('body[id="nv_forum"][class^="pg_"][onkeydown*="27"]').className
      ) {
    ...
    if (curSite.SiteTypeID === 0) {
      if (
        indexOF('search') ||
        getCSS('body[id="nv_search"][onkeydown*="27"]')
      ) {
        if (indexOF('mobile=2', 's')) {
          // 手机版页面
          curSite = DBSite.discuz_m
          curSite.pager.pageE = '.threadlist > ul > li'
          urlC = true
    ...
    if (curSite.SiteTypeID === 0) {
      if (indexOF('.html')) {
        ...
        if (indexOF('/forum-')) {
          ...
          if (
            getXpath(
              '//head/meta[@name="applicable-device" and @content="mobile"] | //head/title[contains(text(), "手机版")] | //head/link[contains(@href, "/mobile/")] | //head/script[contains(@src, "/mobile/")]',
            )
          ) {
            // 手机版页面
            discuzForum('m')
    ...
    // 如果上面没有匹配的则继续
    if (curSite.SiteTypeID === 0) {
      if (indexOF('mod=forumdisplay', 's') || indexOF('forumdisplay.php')) {
        //      < 各版块帖子列表 >
        if (
          indexOF('mobile=2', 's') ||
          indexOF('mobile=yes', 's') ||
          getXpath(
            '//head/meta[@name="applicable-device" and @content="mobile"] | //head/title[contains(text(), "手机版")] | //head/link[contains(@href, "/mobile/")] | //head/script[contains(@src, "/mobile/")]',
          )
        ) {
      ...
      } else if (indexOF('mod=viewthread', 's') || indexOF('viewthread.php')) {
        // < 帖子内 >
        if (
          indexOF('mobile=2', 's') ||
          getXpath(
      ...
      } else if (indexOF('mod=guide', 's')) {
        //      < 导读帖子列表 >
        curSite = DBSite.discuz_guide
      } else if (indexOF('mod=space', 's') && indexOF('do=thread', 's')) {
        // 别人的主题/回复
        curSite = DBSite.discuz_youspace
      } else if (indexOF('mod=collection', 's')) {
        // < 淘贴列表 >
        curSite = DBSite.discuz_collection
      } else if (getCSS('#threadlist, tbody[id^="normalthread_"]')) {
        //          < 部分论坛的各板块 URL 是自定义的 >
        discuzForum()
      } else if (getCSS('#postlist, form>.viewthread')) {
        //            < 部分论坛的帖子内 URL 是自定义的 >
        curSite = DBSite.discuz_thread
      } else if (isMobile()) {
        //                     手机版判断
  ...
  function waterfallStyle() {
    ...
    if (width)
      insStyle(
        `#waterfall {height: auto !important; width: 100% !important;} #waterfall > li {width: ${width} !important; float: left !important; position: inherit !important; left: auto !important; top: auto !important;}`,
      )
  }

  // [NexusMods] 获取下一页地址
  function nexusmods_nextL() {
    if (getCSS('.nexus-ui-blocker') || !getCSS('.pagination a.page-selected'))
      return
    let modList
    if (indexOF('/news')) {
      modList = RH_NewsTabContent
    } else if (indexOF('/users/') && indexOF('tab=user+files', 's')) {
      modList = RH_UserModsTab
    } else if (indexOF('/mods/') && indexOF('tab=posts', 's')) {
      modList = RH_CommentContainer
    } else {
      modList = RH_ModList
    }
    if (!modList) return
    let out_items = JSON.stringify(modList.out_items).replace(/{|}|"/g, ''),
      nextNum = getXpath(
        '//div[contains(@class, "pagination")][1]//a[contains(@class, "page-selected")]/parent::li/following-sibling::li[1]/a',
      )
    var url = ''
    if (nextNum && nextNum.innerText) {
      ...
      if (!indexOF(/\/(mods|users)\/\d+/)) {
        // MOD 页/用户页 不需要这些
        let categories = modList.out_items.categories,
          tags_yes = modList.out_items.tags_yes,
          search = modList.out_items.search,
          out_items_sub = ''
        if (categories && categories != []) {
          // 分类页
          if (modList.out_items.categories instanceof Array) {
            // 单独使用时为数组
            for (let i = 0; i < categories.length; i++) {
              out_items_sub += `categories[]:${categories[i]},`
            }
          } else {
            for (let key in modList.out_items.categories) {
              out_items_sub += `categories[${key}]:${modList.out_items.categories[key]},`
            }
          }
          if (out_items.indexOf('categories:') > -1)
            out_items = out_items.replace('categories:', out_items_sub)
        }
        out_items_sub = ''
        if (tags_yes && tags_yes != []) {
          // 标签页
          if (modList.out_items.tags_yes instanceof Array) {
            // 单独使用时为数组
            for (let i = 0; i < tags_yes.length; i++) {
              out_items_sub += `tags_yes[]:${tags_yes[i]},`
            }
          } else {
            for (let key in modList.out_items.tags_yes) {
              out_items_sub += `tags_yes[${key}]:${modList.out_items.tags_yes[key]},`
            }
          }
          if (out_items.indexOf('tags_yes:') > -1)
            out_items = out_items.replace('tags_yes:', out_items_sub)
        }
        out_items_sub = ''
        if (search && search.length != 0) {
          // 搜索页
          for (let key in modList.out_items.search) {
            out_items_sub += `search[${key}]:${modList.out_items.search[key]},`
          }
          if (out_items.indexOf('search:') > -1)
            out_items = out_items.replace('search:', out_items_sub)
        }
        out_items_sub = ''
      }
  ...
  function nexusmods_bF(pageE) {
    pageE.forEach(function (one) {
      ...
      if (now) {
        let downloadCount = now.querySelector(
          '.downloadcount > span.flex-label',
        )
        if (downloadCount) {
          if (
            GlobalModStats[now.dataset.gameId] &&
            GlobalModStats[now.dataset.gameId][now.dataset.modId]
          ) {
            downloadCount.textContent = shortFormat(
              parseInt(
                GlobalModStats[now.dataset.gameId][now.dataset.modId].total,
              ),
            )
          }
        }
  ...
  function bilibili_search_nextL() {
    ...
    let pageActive = 1,
      pageLast = parseInt(
        getXpath(
          '//ul[@class="pages"]/li[contains(@class, "page-item")][not(contains(@class, "next") or contains(@class, "prev"))][last()]',
        ).innerText,
      )
    if (!pageLast) return
    if (indexOF(/page=\d+/, 's')) {
      pageActive = parseInt(
        /page=\d+/.exec(location.search)[0].replace('page=', ''),
      )
    }
    if (pageActive < pageLast) {
      if (indexOF(/page=\d+/, 's')) {
        return (
          location.origin +
          location.pathname +
          location.search.replace(/page=\d+/, `page=${pageActive + 1}`)
        )
      } else {
        return (
          location.origin +
  ...
  function manhuacat_nextL(pageE, type) {
    if (type === 'url') {
      ...
      if (vg_r_data) {
        getPageE_(
          `${location.origin}/chapter_num?chapter_id=${vg_r_data.dataset.chapter_num}&ctype=1&type=${vg_r_data.dataset.chapterType};`,
          'json',
          'GET',
          '',
          'url',
  ...
  function manhuacat_insertE(pageE, type) {
    ...
    // 插入图片
    let _img = '',
      _img_arr = LZString.decompressFromBase64(
        getXpath(
          '//body/script[not(@src)][contains(text(), "img_data")]',
        ).textContent.split('"')[1],
      ).split(','),
      vg_r_data = getCSS('.vg-r-data')
    for (let now of _img_arr) {
      _img += `<img src="${vg_r_data.dataset.chapterDomain}${img_pre}${now}">`
    }
    if (_img) {
      getOne(curSite.pager.insertP[0]).insertAdjacentHTML(
        getAddTo(curSite.pager.insertP[1]),
        _img,
      ) // 将 img 标签插入到网页中
      ...
      if (one.substring(0, 1) == '/') {
        ...
        if (qTcms_Pic_m_if != '2') {
          ...
          if (typeof qTcms_S_m_mhttpurl != 'undefined')
            m_httpurl = base64_decode(qTcms_S_m_mhttpurl)
          if (location.hostname == 'www.ccshwy.com')
            qTcms_m_indexurl = 'http://h.ccshwy.com/'
          _imgUrl =
            qTcms_m_indexurl +
            'statics/pic/?p=' +
            escape(one) +
  ...
  function imanhuaw_insertE(pageE, type) {
    ...
    // 插入并运行 <script>
    insScript(
      '//head/script[not(@src)][contains(text(), "qTcms_S_m_murl_e")]',
      pageE,
    )
    // 将 img 标签插入到网页中
    getOne(curSite.pager.insertP[0]).insertAdjacentHTML(
  ...
  function hicomic_init() {
    let _img = ''
    getAllCSS('.chapter > section:not(:first-child) > section[val]').forEach(
      function (one) {
    ...
    window.document.title = window.document.title.replace(
      /(\(第.+\))? - HiComic/,
      `(${getCSS('.chapter_name').textContent}) - HiComic`,
    ) // 修改网页标题（加上 第 X 话）
  }
  // [HiComic(嗨漫画)] 获取下一页地址
  function hicomic_nextL() {
    ...
    if (nextId && nextId.id && nextId.id != 'None') {
      ...
      getPageE_(
        `https://www.hicomic.net/api/web/chapter/${nextId.id}/contents`,
        'json',
      )
    }
  }
  ...
  function hicomic_insertE(pageE, type) {
    ...
    addHistory(
      pageE,
      window.document.title.replace(
        /(\(第.+\))? - HiComic/,
        `(${pageE.results.chapter.name}) - HiComic`,
      ),
    )
    let _img = ''
    for (let i = 0; i < pageE.results.chapter.contents.length; i++) {
  ...
  function alimanhua_nextL(pageE, type) {
    if (type === 'url') {
      ...
      getPageE_(
        `${location.origin}/e/extend/ret_page/mindex.php?bid=${
          window.location.href.split('/')[4]
        }&id=${window.location.href.split('/')[5]}&u=1`,
        'json',
        'GET',
        '',
        'url',
  ...
  function alimanhua_insertE(pageE, type) {
    ...
    // 插入图片
    let _img = '',
      _host = 'http://res.img.tueqi.com/'
    if (location.hostname == '') _host = 'https://res.img.96youhuiquan.com/'
    for (let i = 1; i < photosr.length; i++) {
      _img += `<img src="${_host}${photosr[i]}">`
    }
    getCSS(curSite.pager.insertP[0]).insertAdjacentHTML(
      getAddTo(curSite.pager.insertP[1]),
      _img,
    )
    addHistory(pageE)
  ...
  function mangabz_init() {
    if (getCSS('#showimage'))
      getCSS('#showimage').removeAttribute('oncontextmenu')
    if (getCSS('#cp_img')) getCSS('#cp_img').removeAttribute('oncontextmenu')
    if (getCSS('#barChapter'))
      getCSS('#barChapter').removeAttribute('oncontextmenu')
    if (getCSS('#cp_image')) {
  ...
  function mangabz_nextL() {
    ...
    if (MANGABZ_PAGE === MANGABZ_IMAGE_COUNT) {
      // 下一话
      if (getNextE('//a[./img[contains(@src, "icon_xiayizhang")]]'))
        getPageE_(curSite.pageUrl) // 访问下一话 URL 获取
    } else {
      // 下一页
      if (!mkey) var mkey = ''
      url =
        ...
        location.pathname +
        'chapterimage.ashx' +
        `?cid=${MANGABZ_CID}&page=${
          MANGABZ_PAGE + 1
        }&key=${mkey}&_cid=${MANGABZ_CID}&_mid=${MANGABZ_MID}&_dt=${MANGABZ_VIEWSIGN_DT}&_sign=${MANGABZ_VIEWSIGN}`
      ...
      if (type === 'Next') {
        ...
        if (_img) {
          ...
          addHistory(
            ...
            document.title,
            location.origin +
              MANGABZ_CURL.substring(0, MANGABZ_CURL.length - 1) +
              '-p' +
              MANGABZ_PAGE +
              '/',
          )
  ...
  function dm5_nextL() {
    ...
    if (DM5_PAGE === DM5_IMAGE_COUNT) {
      // 下一话
      if (getNextE('//div[@class="view-paging"]//a[text()="下一章"]'))
        getPageE_(curSite.pageUrl) // 访问下一话 URL 获取
    } else {
      // 下一页
      if (!mkey) var mkey = ''
      url =
        ...
        `?cid=${DM5_CID}&page=${
          DM5_PAGE + 1
        }&key=${mkey}&language=1&gtk=6&_cid=${DM5_CID}&_mid=${DM5_MID}&_dt=${DM5_VIEWSIGN_DT}&_sign=${DM5_VIEWSIGN}`
      if (url === curSite.pageUrl) return
      curSite.pageUrl = url
      //console.log(curSite.pageUrl)
      getPageE_(curSite.pageUrl, 'text', 'GET', '', 'Next') // 访问下一页 URL 获取
      ...
      if (type === 'Next') {
        ...
        if (_img) {
          ...
          addHistory(
            ...
            document.title,
            location.origin +
              DM5_CURL.substring(0, DM5_CURL.length - 1) +
              '-p' +
              DM5_PAGE +
              '/',
          )
  ...
  function xmanhua_nextL() {
    ...
    if (XMANHUA_PAGE === XMANHUA_IMAGE_COUNT) {
      // 下一话
      if (getNextE('//a[./img[contains(@src, "reader-bottom-right-2.png")]]'))
        getPageE_(curSite.pageUrl) // 访问下一话 URL 获取
    } else {
      // 下一页
      if (!mkey) var mkey = ''
      url =
        ...
        location.pathname +
        'chapterimage.ashx' +
        `?cid=${XMANHUA_CID}&page=${
          XMANHUA_PAGE + 1
        }&key=${mkey}&_cid=${XMANHUA_CID}&_mid=${XMANHUA_MID}&_dt=${XMANHUA_VIEWSIGN_DT}&_sign=${XMANHUA_VIEWSIGN}`
      if (url === curSite.pageUrl) return
      curSite.pageUrl = url
      //console.log(curSite.pageUrl)
      getPageE_(curSite.pageUrl, 'text', 'GET', '', 'Next') // 访问下一页 URL 获取
      ...
      if (type === 'Next') {
        ...
        if (_img) {
          ...
          addHistory(
            ...
            document.title,
            location.origin +
              XMANHUA_CURL.substring(0, XMANHUA_CURL.length - 1) +
              '-p' +
              XMANHUA_PAGE +
              '/',
          )
  ...
  function nsfc_nextL() {
    ...
    getPageE_(
      location.origin + '/baseQuery/data/completeProjectReport',
      'json',
      'POST',
      data,
    ) // 访问下一页 URL 获取
  ...
  function nsfc_insertE(pageE, type) {
    if (!pageE || pageE.code != 200) {
      curSite.SiteTypeID = 0
      return
    }
    if (!pageE.data.hasnext) {
      curSite.SiteTypeID = 0
    } else {
      document.nowPageNum++
    }
    ...
    getOne(curSite.pager.insertP[0]).insertAdjacentHTML(
      getAddTo(curSite.pager.insertP[1]),
      `<img style="width: 100%;" data-magnify="gallery" data-src="${pageE.data.url}" src="${pageE.data.url}">`,
    )
  }
  ...
  function coolkeyan_nextL() {
    ...
    let id = getCSS('.q-breadcrumbs--last > span.q-breadcrumbs__el'),
      data
    ...
    curSite.pageUrl = data
    getPageE_(
      location.origin + '/api/funds/nsfc/creport?' + data,
      'json',
      'GET',
    ) // 访问下一页 URL 获取
  }
  ...
  function coolkeyan_insertE(pageE, type) {
    ...
    pageNum.now = pageNum._now + 1
    getCSS('.q-img>div[style*="padding-bottom"]').style.paddingBottom = `${
      document.nowPageNum * 1000 - 1000
    }px`
    getOne(curSite.pager.insertP[0]).insertAdjacentHTML(
      getAddTo(curSite.pager.insertP[1]),
      `<div class="q-img__image absolute-full" style="background-size: contain; background-position: 50% 50%; background-image: url('${
        pageE.url
      }'); top: ${document.nowPageNum * 1000 - 2000}px"></div>`,
    )
  }
  ...
  function pageLoading() {
    ...
    windowScroll(function (direction, e) {
      ...
      if (
        curSite.pager.type == 5 &&
        self != top &&
        window.top.document.xiu_pausePage == false
      )
        return
      ...
      if (curSite.pager.type === 3) {
        if (!curSite.pager.scrollE)
          curSite.pager.scrollE = curSite.pager.replaceE // 默认基准元素是页码
        ...
        scrollHeight + scrollTop + scrollD
      ) {
        // <<<<< 翻页类型 1（由脚本实现自动无缝翻页）>>>>>
        if (curSite.pager.type === 1) {
          ...
          if (typeof curSite.pager.nextL == 'function') {
            curSite.pager.nextL()
          } else if (getNextE(curSite.pager.nextL)) {
            getPageE_(curSite.pageUrl)
          }
  ...
  function getPageE(url) {
    GM_xmlhttpRequest({
      ...
      ontimeout: function (response) {
        ...
        console.log('URL：' + url, response)
        GM_notification({
          text: '❌ 获取下一页超时，可 3 秒后再次滚动网页重试（或尝试刷新网页）...',
          timeout: 5000,
        })
      },
    })
        ...
        try {
          ...
          switch (type) {
            ...
            case 'text':
              ...
              break
            default:
              curSite.pager.insertE(
                createDocumentByString(response.responseText),
                type2,
              )
          }
        } catch (e) {
  ...
  function insIframe(src) {
    ...
    document.documentElement.appendChild(
      ...
      'iframe#Autopage_iframe {position: absolute !important; width: 100% !important; height: 100% !important; border: none !important;}'
    ...
    window.addEventListener(
      ...
      function () {
        ...
        if (
          ...
          !getCSS('#Autopage_iframe-scroll')
        ) {
          let newStyle = document.createElement('style')
          newStyle.id = 'Autopage_iframe-scroll'
          newStyle.textContent =
            'html::-webkit-scrollbar, body::-webkit-scrollbar {width: 0 !important;height: 0 !important;} html, body {scrollbar-width: none !important;}'
          ...
          if (
            iframe.contentWindow.document.querySelectorAll(
              '#Autopage_iframe-scroll-hidden',
            )
          )
            iframe.contentWindow.document
              .querySelectorAll('#Autopage_iframe-scroll-hidden')
              .forEach((o) => {
                o.remove()
              })
          ...
          if (
            ...
            let newStyle = document.createElement('style')
            newStyle.id = 'Autopage_iframe-scroll-hidden'
    ...
    iframe.onload = function () {
      ...
      if (curSite.history === undefined) {
        if (GM_getValue('menu_history', true))
          addHistory(
            iframe.contentWindow.document,
      ...
      if (!curSite.hiddenPN) {
        ...
        if (autopageNumber) {
          autopageNumber.textContent = parseInt(autopageNumber.textContent) + 1
        }
      }
    }
  ...
  function insIframe_(src) {
    ...
    if (!iframe) {
      ...
      document.documentElement.appendChild(
        ...
        'iframe#Autopage_iframe {position: absolute !important; top: -9999px !important; left: -9999px !important; width: 100% !important; height: 100% !important; border: none !important; z-index: -999 !important;}'
    }
  ...
  function processElems(response) {
    if (!curSite.pager.insertP) {
      curSite.pager.insertP = [curSite.pager.pageE, 5]
    }
    let pageE = getAll(curSite.pager.pageE, response, response),
      toE
    if (curSite.pager.insertP[1] === 5) {
      // 插入 pageE 列表最后一个元素的后面
    ...
    if (pageE.length > 0 && toE) {
      ...
      if (curSite.function && curSite.function.bF) {
        if (curSite.function.bFp) {
          ...
          if (typeof curSite.function.bF == 'string') {
            // 如果是字符串，说明是自定义规则
            pageE = new Function('pageE', 'bFp', 'fun', curSite.function.bF)(
              pageE,
              curSite.function.bFp,
              window.autoPage,
            )
          } else {
          ...
          if (typeof curSite.function.bF == 'string') {
            // 如果是字符串，说明是自定义规则
            pageE = new Function('pageE', 'fun', curSite.function.bF)(
              pageE,
              window.autoPage,
            )
          } else {
      ...
      if (curSite.pager.insertP[1] === 6) {
        // 插入到目标内部末尾（针对文本，比如小说网页）
        let afterend = ''
        if (curSite.pager.insertP6Br) afterend += '<br/><br/>'
        pageE.forEach(function (one) {
          afterend += one.innerHTML
        })
        toE.insertAdjacentHTML(addTo, afterend)
      } else {
        if (
          curSite.pager.insertP[1] === 2 ||
          curSite.pager.insertP[1] === 4 ||
          curSite.pager.insertP[1] === 5
        )
          pageE.reverse() // 插入到 [元素内头部]、[目标本身后面] 时，需要反转顺序
        pageE.forEach(function (one) {
          toE.insertAdjacentElement(addTo, one)
        })
      }
      ...
      if (curSite.pager.scriptT || curSite.pager.scriptT == 0) {
        switch (curSite.pager.scriptT) {
          case 0: // 下一页的所有 <script> 标签
            insScript('script', response)
            break
          case 1: // 下一页的所有 <script> 标签（不包括 src 链接）
            insScript('script:not([src])', response)
            break
          case 2: // 下一页主体元素 (pageE) 的同级 <script> 标签
            if (
              ...
              pageE.reverse() // 为了避免 JS 执行顺序反了，还需要再给反转回去
            insScript(null, pageE)
            break
          case 3: // 下一页主体元素 (pageE) 的子元素 <script> 标签
            if (
              ...
              pageE.reverse() // 为了避免 JS 执行顺序反了，还需要再给反转回去
            insScript('script:not([src])', pageE)
            break
        }
      }
      ...
      if (curSite.function && curSite.function.aF) {
        if (curSite.function.aFp) {
          ...
          if (typeof curSite.function.aF == 'string') {
            // 如果是字符串，说明是自定义规则
            new Function('aFp', 'fun', curSite.function.aF)(
              curSite.function.aFp,
              window.autoPage,
            )
          } else {
  ...
  function src_bF(
    pageE,
    css = [0, 'img[data-original], img[data-src]', 'data-original'],
  ) {
    ...
    pageE.forEach(function (one) {
      if (css[0] == 0) {
        // src 图片
        if (one.tagName === 'IMG') {
          if (one.getAttribute(css[2])) {
            one.src = one.getAttribute(css[2])
          } else if (one.dataset.src) {
            one.src = one.dataset.src
          }
        } else {
          one.querySelectorAll(css[1]).forEach(function (now) {
            if (now.getAttribute(css[2])) {
              now.src = now.getAttribute(css[2])
            } else if (now.dataset.src) {
        ...
        if (one.tagName === 'IMG') {
          if (one.getAttribute(css[2])) {
            one.style.backgroundImage =
              'url("' + one.getAttribute(css[2]) + '")'
          } else if (one.dataset.src) {
            one.style.backgroundImage = 'url("' + one.dataset.src + '")'
          }
        } else {
          one.querySelectorAll(css[1]).forEach(function (now) {
            if (now.getAttribute(css[2])) {
              now.style.backgroundImage =
                'url("' + now.getAttribute(css[2]) + '")'
            } else if (now.dataset.src) {
              now.style.backgroundImage = 'url("' + now.dataset.src + '")'
            }
          })
        }
      }
  ...
  function getNextE(css) {
    ...
    if (
      ...
      next.nodeType === 1 &&
      next.href &&
      next.href.slice(0, 4) === 'http' &&
      next.getAttribute('href').slice(0, 1) !== '#'
    ) {
  ...
  function checkURL(func) {
    ...
    if (typeof curSite.pager.nextL == 'function') {
      ...
      if (
        !tempUrl ||
        tempUrl === curSite.pageUrl ||
        tempUrl.slice(0, 4) !== 'http'
      )
        return
      curSite.pageUrl = tempUrl
      func(curSite.pageUrl)
    } else if (curSite.pager.nextL.search(/^js;/i) === 0) {
      // 自定义翻页规则中执行 JavaScript 代码的
      try {
        let tempUrl = new Function('fun', curSite.pager.nextL.slice(3))(
          window.autoPage,
        )
        if (
          !tempUrl ||
        ...
        console.error(
          '[自动无缝翻页] - 当前网页规则 "nextL" 内 JS 代码有误，请检查！',
          e,
          curSite.pager.nextL,
        )
      }
  ...
  function replaceElems(
    pageE,
    o = curSite.pager.replaceE,
    r = curSite.pager.replaceE,
  ) {
    let oE = getAll(o),
  ...
  function addHistory(pageE, title, url) {
    ...
    title =
      title ||
      (pageE.querySelector('title')
        ? pageE.querySelector('title').textContent
    ...
    if (url.indexOf(window.top.location.protocol) === -1)
      url = url.replace(/^https?:/, window.top.location.protocol)
    ...
    scriptElems.forEach(function (one) {
      if (one.tagName === 'SCRIPT') {
        if (one.src) {
          ...
          toE.appendChild(document.createElement('script')).textContent =
            one.textContent //.replaceAll('document.write', '');
        }
      }
    })
  }
  ...
  function createDocumentByString(e) {
    if (e) {
      ...
      if (document.implementation.createHTMLDocument) {
        ...
        try {
          ;(t = document.cloneNode(!1)).appendChild(
            t.importNode(document.documentElement, !1),
          ),
      ...
      if (t) {
        ...
        for (
          var a,
            o = { TITLE: !0, META: !0, LINK: !0, STYLE: !0, BASE: !0 },
            i = t.body,
            s = i.childNodes,
            c = s.length - 1;
          c >= 0;
  ...
  function forceTarget(pageE) {
    if (curSite.blank === 1) {
      document.head.appendChild(document.createElement('base')).target =
        '_blank'
    } else if (curSite.blank === 4) {
      if (pageE) {
        pageE.forEach(function (dd) {
          getAllCSS(
            'a[href]:not([target="_blank"]):not([onclick]):not([href^="#"]):not([href^="javascript:"])',
            dd,
          ).forEach(function (a) {
            a.target = '_blank'
          })
  ...
  function menu_disable(type) {
    ...
    function check() {
      // 存在返回真，不存在返回假
      if (GM_getValue('menu_disable').indexOf(location.hostname) == -1)
        return false // 不存在返回假
      return true
    }
  ...
  function pausePageEvent() {
    ...
    if (curSite.SiteTypeID === 0) return
    if (curSite.pager && curSite.pager.type == 5)
      window.top.document.xiu_pausePage = pausePage
  ...
  function customRules() {
    ...
    let customRules = JSON.stringify(
      GM_getValue('menu_customRules', {}),
      null,
      '\t',
    )
...
<button id="Autopage_customRules_save" style="margin-right: 20px !important;">保存并刷新</button><button id="Autopage_customRules_cancel">取消修改</button>
    ...
    document.documentElement.style.overflow = document.body.style.overflow =
      'hidden' // 避免网页本身滚动
    getCSS('#Autopage_customRules_all').textContent = JSON.stringify(
      DBSite2,
      null,
      '\t',
    ) // 单独插入全部规则列表，避免被 insertAdjacentHTML 语义化 HTML 标签
    ...
    getCSS('#Autopage_customRules_cancel').onclick = function () {
      document.documentElement.style.overflow = document.body.style.overflow =
        ''
  ...
  function pageNumber(type) {
    if (
      curSite.SiteTypeID === 0 ||
      curSite.hiddenPN ||
      (curSite.pager && curSite.pager.type == 5 && self != top)
    ) {
      if (getCSS('#Autopage_number')) {
        getCSS('#Autopage_number').style.display = 'none'
      }
      return
    }
    ...
    function add() {
      ...
      if (location.hostname === 'bbs.pcbeta.com') {
        setTimeout(function () {
          document.documentElement.insertAdjacentHTML('beforeend', _style)
        }, 500)
      }
      if (curSite.pager && curSite.pager.type == 5)
        window.top.document.xiu_pausePage = pausePage
      ...
      getCSS('#Autopage_number').onclick = function (e) {
        if (pausePage) {
          this.style =
            'color: #FF5722 !important; font-style: italic !important;'
        } else {
          this.style = ''
        }
        pausePage = !pausePage
